...
public class MarketplaceWebServiceClient implements MarketplaceWebService {
	...
	/**
	 * Constructs MarketplaceWebServiceClient with AWS Access Key ID and AWS Secret
	 * Key
	 *
	...
	/**
	 * Constructs MarketplaceWebServiceClient with AWS Access Key ID, AWS Secret Key
	 * and MarketplaceWebServiceConfig. Use MarketplaceWebServiceConfig to pass
	 * additional configuration that affects how service is being called.
	 *
	 * @param awsAccessKeyId     AWS Access Key ID
	 * @param awsSecretAccessKey AWS Secret Access Key
	 ...
	 * potentially be very large (>100MB) which is why we only return one report at
	 * a time, and in a streaming fashion.
	 * 
	 * @param request GetReportRequest request
	 ...
	 * <p/>
	 * If service error has occured, MarketplaceWebServiceException can be extracted
	 * with <code>exception.getCause()</code>
	 * <p/>
	 ...
	 * 
	 * @param request GetReportScheduleCountRequest request
	 ...
	 * 
	 * @param request GetReportRequestListByNextTokenRequest request
	 ...
	 *
	 * @param request GetReportRequestListByNextTokenRequest request
	 * @return Future&lt;GetReportRequestListByNextTokenResponse&gt; future pointer
	 *         to GetReportRequestListByNextTokenResponse
	 * 
	 */
	 ...
	 *
	 * The UpdateReportAcknowledgements operation updates the acknowledged status of
	 * one or more reports.
	 * 
	 * @param request UpdateReportAcknowledgementsRequest request
	 ...
	 *
	 * @param request UpdateReportAcknowledgementsRequest request
	 * @return Future&lt;UpdateReportAcknowledgementsResponse&gt; future pointer to
	 *         UpdateReportAcknowledgementsResponse
	 * 
	 */
	 ...
	 * Submit Feed
	 *
	 * Uploads a file for processing together with the necessary metadata to process
	 * the file, such as which type of feed it is. PurgeAndReplace if true means
	 * that your existing e.g. inventory is wiped out and replace with the contents
	 * of this feed - use with caution (the default is false).
	 * 
	 * @param request SubmitFeedRequest request
	 ...
	 *
	 * Uploads a file from disk for processing together with the necessary metadata
	 * to process the file, such as which type of feed it is. PurgeAndReplace if
	 * true means that your existing e.g. inventory is wiped out and replace with
	 * the contents of this feed - use with caution (the default is false).
	 * 
	 ...
	 * on disk, as the assumption is that the content is accessed through a
	 * FileInputStream.
	 * 
	 * @param request SubmitFeedRequest request without the contentMd5 field set.
	 * @return SubmitFeed Response from the service
	 *
	 * @throws MarketplaceWebServiceException
	 */
	 ...
	 * Get Report Count
	 *
	 * returns a count of reports matching your criteria; by default, the number of
	 * reports generated in the last 90 days, regardless of acknowledgment status
	 * 
	 * @param request GetReportCountRequest request
	 ...
	 * retrieve the next batch of list items and if there are more items to retrieve
	 * 
	 * @param request GetFeedSubmissionListByNextTokenRequest request
	 ...
	 *
	 * @param request GetFeedSubmissionListByNextTokenRequest request
	 * @return Future&lt;GetFeedSubmissionListByNextTokenResponse&gt; future pointer
	 ...
	 *
	 * cancels feed submissions - by default all of the submissions of the last 30
	 * days that have not started processing
	 * 
	 * @param request CancelFeedSubmissionsRequest request
	 ...
	 * 
	 * @param request RequestReportRequest request
	 ...
	 * 
	 * @param request GetFeedSubmissionCountRequest request
	 ...
	 *
	 * cancels report request that have not yet started processing, by default all
	 * those within the last 90 days
	 * 
	 * @param request CancelReportRequestsRequest request
	 ...
	 * Get Report List
	 *
	 * returns a list of reports; by default the most recent ten reports, regardless
	 * of their acknowledgment status
	 * 
	 * @param request GetReportListRequest request
	 ...
	 * 
	 * @param request GetFeedSubmissionResultRequest request
	 ...
	 * 
	 * @param request GetFeedSubmissionListRequest request
	 ...
	 * 
	 * @param request GetReportRequestListRequest request
	 ...
	 * retrieve the next batch of list items and if there are more items to retrieve
	 * 
	 * @param request GetReportScheduleListByNextTokenRequest request
	 ...
	 *         to GetReportScheduleListByNextTokenResponse
	 * 
	 */
	 ...
	 * retrieve the next batch of list items and if there are more items to retrieve
	 * 
	 * @param request GetReportListByNextTokenRequest request
	 ...
	 *
	 * Creates, updates, or deletes a report schedule for a given report type, such
	 * as order reports in particular.
	 * 
	 * @param request ManageReportScheduleRequest request
	 ...
	 *
	 * returns a count of report requests; by default all the report requests in the
	 * last 90 days
	 * 
	 * @param request GetReportRequestCountRequest request
	 ...
	 * 
	 * @param request GetReportScheduleListRequest request
	...
	/**
	 * Remove all leading whitespace, trailing whitespace, repeated whitespace and
	 * replace any interior whitespace with a single space
	 */
	...
	private <T, U> T invoke(Class<T> clazz, Map<String, String> parameters, U request)
		...
		try {
			...
			// SubmitFeed will be the only MWS API function that will stream requests to the
			// server.
			if (request instanceof SubmitFeedRequest) {

				// For SubmitFeed, HTTP body is reserved for the Feed Content and the function
				// parameters
				...
				/*
				 * Set content type and encoding - encoding and charset are ignored right now
				 * because octet-stream is the only supported transport of MWS feeds.
				 */
		...
		try {
			...
			do {
				...
				try {
					...
					// GetFeedSubmissionResult and GetReport will be the only MWS API functions that
					// will stream
					// server responses.
					...
					/*
					 * Response cannot be unmarshalled neither as <Action>Response or ErrorResponse
	...
	private ResponseHeaderMetadata getResponseHeaderMetadata(HttpResponse response) {
		...
		Header maxQuota = null;
		Header quotaRemaining = null;
		Header quotaResetAt = null;
		...
		maxQuota = getFirstHeader(response, "x-mws-quota-max");
		quotaRemaining = getFirstHeader(response, "x-mws-quota-remaining");
		quotaResetAt = getFirstHeader(response, "x-mws-quota-resetsOn");
		...
		return new ResponseHeaderMetadata(requestId != null ? requestId.getValue() : null,
				...
				timestamp != null ? timestamp.getValue() : null, maxQuota != null ? maxQuota.getValue() : null,
				quotaRemaining != null ? quotaRemaining.getValue() : null,
				quotaResetAt != null ? quotaResetAt.getValue() : null);
	}
	...
	/**
	 * Add authentication related and version parameter and set request body with
	...
	/**
	 * Computes RFC 2104-compliant HMAC signature for request parameters Implements
	 * AWS Signature, as per following spec:
	 *
	 * If Signature Version is 1, it performs the following:
	 *
	 * Sorts all parameters (including SignatureVersion and excluding Signature, the
	 * value of which is being created), ignoring case.
	 *
	 ...
	 * Host header in the form of lowercase host, followed by an ASCII newline. 3.
	 * The URL encoded HTTP absolute path component of the URI (up to but not
	 * including the query string parameters); if this is empty use a forward '/'.
	 * This parameter is followed by an ASCII newline. 4. The concatenation of all
	 * query string components (names and values) as UTF-8 characters which are URL
	 * encoded as per RFC 3986 (hex characters MUST be uppercase), sorted using
	 * lexicographic byte ordering. Parameter names are separated from their values
	 * by the '=' character (ASCII character 61), even if the value is empty. Pairs
	 * of parameter and values are separated by the '&' character (ASCII code 38).
	 *
	 */
